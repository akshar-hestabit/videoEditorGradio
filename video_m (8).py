...
def parse_prompt_with_openai(prompt: str) -> VideoEditParams:
    ...
    examples = [
        ...
        {
            "prompt": "take off 7 seconds from video 1 and add to the back of video two",
            "output": '{"operation": "append_to_back", "cut_duration": 7.0, "replace_duration": null, "cut_location": "start", "replace_location": "end", "cut_position": null, "cut_start": null, "replace_start": null}'
        },
        {
        "prompt": "Take the first 7 seconds from video 1 and merge with video 2",
        "output": '{"operation": "join", "cut_duration": 7.0, "replace_duration": 0.0, "cut_location": "start", "replace_location": "start", "cut_position": null, "cut_start": null, "replace_start": null}'
        },
        {
        "prompt": "Use first 7 seconds of video1 and then add full video2",
        "output": '{"operation": "join", "cut_duration": 7.0, "replace_duration": 0.0, "cut_location": "start", "replace_location": "start", "cut_position": null, "cut_start": null, "replace_start": null}'
        },
        {
          "prompt": "Take 5 seconds from video 1 and add in video 2 after 5 seconds",
          "output": '{"operation": "insert_at", "cut_duration": 5.0, "replace_duration": 0.0, "cut_location": "start", "replace_location": "specific", "cut_start": null, "replace_start": 5.0, "cut_position": null}'
        },
        {
          "prompt": "Take 5 seconds from video 2 and insert into video 1 after 10 seconds",
          "output": '{"operation": "insert_at", "cut_duration": 5.0, "replace_duration": 0.0, "cut_location": "start", "replace_location": "specific", "cut_start": null, "replace_start": 10.0, "cut_position": null}'
        },

        {
          "prompt": "Take 2 to 7 seconds from video 2 and merge with video 1",
          "output": "{\"operation\": \"join\", \"cut_duration\": 5.0, \"replace_duration\": 0.0, \"cut_location\": \"specific\", \"replace_location\": \"start\", \"cut_start\": 2.0, \"replace_start\": null, \"cut_position\": null, \"source_video\": \"video2\", \"target_video\": \"video1\"}"
        },
        {
          "prompt": "Take 5 seconds from video 2 and append to the end of video 1",
          "output": "{\"operation\": \"append_to_back\", \"cut_duration\": 5.0, \"replace_duration\": null, \"cut_location\": \"start\", \"replace_location\": \"end\", \"cut_start\": null, \"replace_start\": null, \"cut_position\": null, \"source_video\": \"video2\", \"target_video\": \"video1\"}"
        },
        {
          "prompt": "Insert the first 4 seconds of video 2 into video 1 after 6 seconds",
          "output": "{\"operation\": \"insert_at\", \"cut_duration\": 4.0, \"replace_duration\": 0.0, \"cut_location\": \"start\", \"replace_location\": \"specific\", \"cut_start\": null, \"replace_start\": 6.0, \"cut_position\": null, \"source_video\": \"video2\", \"target_video\": \"video1\"}"
        },
        {
          "prompt": "Replace the middle 6 seconds of video 1 with the first 6 seconds of video 2",
          "output": "{\"operation\": \"cut_and_replace\", \"cut_duration\": 6.0, \"replace_duration\": 6.0, \"cut_location\": \"middle\", \"replace_location\": \"start\", \"cut_start\": null, \"replace_start\": null, \"cut_position\": null, \"source_video\": \"video2\", \"target_video\": \"video1\"}"
        },
        {
          "prompt": "Replace from 5 to 10 seconds in video 1 with from 1 to 6 seconds in video 2",
          "output": "{\"operation\": \"cut_and_replace\", \"cut_duration\": 5.0, \"replace_duration\": 5.0, \"cut_location\": \"specific\", \"replace_location\": \"specific\", \"cut_start\": 5.0, \"replace_start\": 1.0, \"cut_position\": null, \"source_video\": \"video2\", \"target_video\": \"video1\"}"
        },
        {
          "prompt": "Add 4 seconds from 3 to 7 in video 2 to video 1 after 8 seconds",
          "output": "{\"operation\": \"insert_at\", \"cut_duration\": 4.0, \"cut_location\": \"specific\", \"cut_start\": 3.0, \"replace_location\": \"specific\", \"replace_start\": 8.0, \"replace_duration\": 0.0, \"cut_position\": null, \"source_video\": \"video2\", \"target_video\": \"video1\"}"
        },
...
- If the prompt is to add a clip from one video to the back of another, the operation should be 'append_to_back'.
- Always include 'source_video' and 'target_video' in your output based on prompt order.
"""
        }
    ]
    ...
    try:
        ...
        if json_match:
            ...
            try:
                ...
                params_dict["source_video"] = params_dict.get("source_video", "video1")
                params_dict["target_video"] = params_dict.get("target_video", "video2")
...
def generate_confirmation_message(params: VideoEditParams) -> str:
    ...
    if params.operation == "join":
        if params.cut_duration > 0:
            if params.cut_location == "specific" and params.cut_start is not None:
                end_time = params.cut_start + params.cut_duration
                message = (
                    f"I will take {params.cut_duration:.1f} seconds from Video 1 "
                    f"(from {params.cut_start:.1f}s to {end_time:.1f}s) "
                    f"and join it with the full Video 2."
                )
            else:
                message = (
                    f"I will take the first {params.cut_duration:.1f} seconds "
                    f"from Video 1 and join it with the full Video 2."
                )
        else:
    ...
    if params.operation == "append_to_back":
        if params.cut_location == "specific" and params.cut_start is not None:
            start = params.cut_start
            end = start + params.cut_duration
            message = (
                f"I will take {params.cut_duration:.1f} seconds from Video 1 "
                f"(from {start:.1f}s to {end:.1f}s) and append it to the end of Video 2."
            )
        else:
            message = f"I will take the first {params.cut_duration:.1f} seconds from Video 1 and add it to the end of Video 2."
        print(f"[generate_confirmation_message] Generated message: {message}")
        return message

    # Default: cut_and_replace
    ...
    if params.operation == "append_to_back":
    # If user specified a specific range, mention the from–to times
        if params.cut_location == "specific" and params.cut_start is not None:
            start = params.cut_start
            end = start + params.cut_duration
            message = (
...
def safe_clip_processing(clip1, clip2, params: VideoEditParams):
    ...
    clip_source = clip1 if params.source_video == "video1" else clip2
    clip_target = clip2 if params.target_video == "video2" else clip1
    ...
    try:
        ...
        if replace_duration and replace_duration > clip2.duration:
        ...
        if params.operation == "cut_and_replace":
            if params.cut_location == "specific":
                ...
                if cut_position + params.cut_duration > clip1.duration:
                    ...
                    raise ValueError("Cut position and duration exceed video length")
            ...
            if params.cut_duration > 0:
                print(f"[safe_clip_processing] Joining first {params.cut_duration}s of video1 with full video2")
                if params.cut_duration > clip1.duration:
                    print(f"[safe_clip_processing] Error: Cut duration {params.cut_duration}s exceeds video1 length {clip1.duration:.1f}s")
                    raise ValueError(f"Cut duration exceeds video1 length")
                
                best_frame = find_best_scene_cut(clip1, params.cut_duration, fps)
                t_end = best_frame / fps
                clip1_segment = clip1.subclip(0, t_end)
                clip1_segment = match_video_properties(clip1_segment, clip2)
                clips_to_join = [clip1_segment, clip2]
                print(f"[safe_clip_processing] Joining clips: [video1_segment ({t_end:.3f}s), video2]")
            else:
        ...
        elif params.operation == "append_to_back":
            print(f"[safe_clip_processing] Appending first {params.cut_duration}s of video1 to the end of video2")
            best_frame = find_best_scene_cut(clip1, params.cut_duration, fps)
            t_cut = best_frame / fps
            clip1_segment = clip1.subclip(0, t_cut)
            clip1_segment_matched = match_video_properties(clip1_segment, clip2)
            clips_to_join = [clip2, clip1_segment_matched]
            final_clip = concatenate_videoclips(clips_to_join, method="compose")
            print(f"[safe_clip_processing] Appended clip from video1 to video2")

        elif params.operation == "insert_at":
            if params.replace_location != "specific" or params.replace_start is None:
                raise ValueError("replace_start must be provided for insert_at operation")
            ...
            # split clip2 at replace_start
            replace_time = params.replace_start
            clip2_before = clip2.subclip(0, replace_time)
            clip2_after = clip2.subclip(replace_time)
        
            clip1_segment = match_video_properties(clip1_segment, clip2)
            clips_to_join = [clip2_before, clip1_segment, clip2_after]
            final_clip = concatenate_videoclips(clips_to_join, method="compose")
            print(f"[safe_clip_processing] Inserted clip1 into video2 at {replace_time}s")
